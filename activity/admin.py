...
BOOK_TITLE = Subquery(
    Book.objects.filter(pk=OuterRef("book_id")).values("title")[:1]  # noqa: E501 pylint: disable=no-member
)
...
class ReadingStatusAdmin(admin.ModelAdmin):
    """Admin interface for tracking user reading activity."""
    list_display = ("user", "book_id", "book_title", "status", "updated_at", "google_link")  # noqa: E501 pylint: disable=line-too-long
    list_filter = ("status",)
